...
export default function CompaniesScreen() {
  ...
  const [form, setForm] = useState({ name: '', url: '', contact_name: '', notes: '' });
  ...
  useEffect(() => {
    const sub = DeviceEventEmitter.addListener('jobVaultRefresh', fetchCompanies);
    return () => sub.remove();
  }, []);
  ...
  return (
    <View style={styles.container}>
      <ScrollView style={styles.list}>
        {companies.length === 0 && (
          <Text style={styles.empty}>No companies yet. Add one to get started.</Text>
        )}
        {companies.map((c) => (
          <View key={c.id} style={[styles.companyCard, c.archived && styles.archivedCard]}>
            ...
            <View style={styles.companyMain}>
              <Text style={[styles.companyName, c.archived && styles.archivedText]}>{c.name}</Text>
              {!!c.url && <Text style={styles.companyMeta}>{c.url}</Text>}
              {!!c.contact_name && (
                <Text style={styles.companyMeta}>Contact: {c.contact_name}</Text>
              )}
            ...
            <View style={styles.companyActions}>
              ...
              <View style={styles.iconActions}>
                <TouchableOpacity onPress={() => handleArchiveToggle(c)} style={styles.editBtn}>
                  {c.archived ? (
                ...
                <TouchableOpacity onPress={() => handleEditStart(c)} style={styles.editBtn}>
      ...
      >
        ...
        >
          <View style={styles.modalHeader}>
            <Text variant="titleMedium">{editingCompany ? 'Edit Company' : 'Add Company'}</Text>
            <TouchableOpacity
              onPress={() => {
                setShowForm(false);
                setEditingCompany(null);
          ...
          <ScrollView style={styles.modalBody}>
            ...
            <TouchableOpacity style={styles.primaryBtn} onPress={handleCreate} disabled={saving}>
              <Text style={styles.primaryBtnText}>
                {saving ? 'Saving...' : editingCompany ? 'Update Company' : 'Save Company'}
              </Text>
            </TouchableOpacity>
          </ScrollView>
        </KeyboardAvoidingView>
...
const StarRating = ({ value, onChange, disabled = false }) => (
  <View style={{ flexDirection: 'row', gap: 2 }}>
    {[1, 2, 3, 4, 5, 6, 7].map((n) => (
      <TouchableOpacity key={n} onPress={() => !disabled && onChange(n)} disabled={disabled}>
        <Star
          size={18}
...
const styles = StyleSheet.create({
  ...
  companyNotes: { fontSize: 13, color: '#374151', fontStyle: 'italic', marginTop: 4 },
  companyActions: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
