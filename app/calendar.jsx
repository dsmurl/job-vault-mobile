...
import { parseEventTime, localInputToUTC, utcToLocalInput } from '../utils/time';
import { ChevronLeft, ChevronRight, Plus, X, CalendarDays } from 'lucide-react-native';
...
export default function CalendarScreen() {
  ...
  const [filterCompanyPickerVisible, setFilterCompanyPickerVisible] = useState(false);
      ...
      headerRight: () => (
        ...
        >
          <Text style={styles.headerFilterText}>
            {selectedCompany
              ? companies.find((c) => c.id.toString() === selectedCompany)?.name || 'All Companies'
              : 'All Companies'}
          </Text>
  ...
  useEffect(() => {
    if (daySheetVisible) {
      ...
      Animated.parallel([
        Animated.timing(sheetOverlayOpacity, { toValue: 0, duration: 250, useNativeDriver: true }),
        Animated.timing(sheetTranslateY, { toValue: 600, duration: 300, useNativeDriver: true }),
      ]).start(() => {
  ...
  const selectedDayEvents = selectedDay
    ? filteredEvents.filter((e) => isSameDay(parseEventTime(e.start_time), selectedDay))
    : [];
  ...
  const isCurrentMonth = isSameDay(startOfMonth(currentDate), startOfMonth(today));
    ...
    <View style={styles.container}>
                ...
                <View style={styles.dotRow}>
                  {dayEvents.slice(0, 3).map((e) => (
                    <View
                      ...
                      style={[
                        styles.dot,
                        { backgroundColor: eventTypeColor(e.event_type, e.company?.archived) },
                      ]}
                    />
                  ))}
                  {dayEvents.length > 3 && (
          ...
          <View style={styles.dialog}>
            ...
            <ScrollView style={styles.dialogBody}>
              <Text style={styles.eventViewMeta}>
                {viewingEvent &&
                  format(parseEventTime(viewingEvent.start_time), 'EEEE, MMMM d, yyyy')}
                {'\n'}
                {viewingEvent && format(parseEventTime(viewingEvent.start_time), 'h:mm a')}
      ...
      >
        ...
        >
          <View style={styles.dialog}>
            <View style={styles.dialogHeader}>
              <Text style={styles.dialogTitle}>{editingEvent ? 'Edit Event' : 'New Event'}</Text>
              <TouchableOpacity
                onPress={() => {
                  setFormDialogVisible(false);
                  setEditingEvent(null);
            ...
            <ScrollView style={styles.dialogBody} keyboardShouldPersistTaps="handled">
              <TextInput
                ...
                onChangeText={(t) => {
                  setForm((f) => ({ ...f, title: t }));
                  if (formErrors.title) setFormErrors((e) => ({ ...e, title: false }));
                }}
              />
                ...
                {EMOJI_OPTIONS.map((opt) => (
                  <TouchableOpacity
                    ...
                    onPress={() =>
                      setForm((f) => ({
                        ...f,
                        selected_emoji: f.selected_emoji === opt.label ? '' : opt.label,
                      }))
                    }
                  >
                    <Text
                      style={[
                        styles.emojiText,
                        form.selected_emoji !== opt.label && styles.emojiTextInactive,
                      ]}
                    >
                      {opt.emoji}
                    </Text>
              ...
              >
                <Text style={styles.formDropdownText}>
                  {EVENT_TYPES.find((t) => t.value === form.event_type)?.label || 'Select type'}
                </Text>
              ...
              >
                <Text style={styles.formDropdownText}>
                  {showNewCompany
                    ? '+ New Company'
                    : form.company_id
                      ? companies.find((c) => c.id.toString() === form.company_id)?.name ||
                        'Select company'
                      : 'None'}
                </Text>
                <ChevronRight size={16} color="#6b7280" />
      ...
      >
        {EVENT_TYPES.map((t) => (
          <TouchableOpacity
            key={t.value}
            style={[styles.pickerOption, form.event_type === t.value && styles.pickerOptionActive]}
            onPress={() => {
      ...
      >
        <TouchableOpacity
          style={[styles.pickerOption, !selectedCompany && styles.pickerOptionActive]}
          onPress={() => {
            setSelectedCompany('');
        ...
        >
          <Text
            style={[styles.pickerOptionText, !selectedCompany && styles.pickerOptionTextActive]}
          >
            All Companies
          </Text>
        </TouchableOpacity>
        {companies.map((c) => (
          ...
          >
            <Text
              style={[
                styles.pickerOptionText,
                selectedCompany === c.id.toString() && styles.pickerOptionTextActive,
      ...
      >
        <TouchableOpacity
          style={[styles.pickerOption, showNewCompany && styles.pickerOptionActive]}
          onPress={() => {
        ...
        >
          <Text
            style={[
              styles.pickerOptionText,
              !form.company_id && !showNewCompany && styles.pickerOptionTextActive,
            ]}
          >
            None
          </Text>
        ...
        {companies.map((c) => (
          <TouchableOpacity
            ...
            style={[
              styles.pickerOption,
              form.company_id === c.id.toString() && !showNewCompany && styles.pickerOptionActive,
              c.archived && styles.pickerOptionArchived,
            ]}
            onPress={() => {
...
const styles = StyleSheet.create({
  ...
  monthLabel: { fontSize: 18, fontWeight: '700', color: '#111827', letterSpacing: 0.2 },
  todayBtn: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: 4,
  ...
  todayBtnText: { fontSize: 11, fontWeight: '700', color: '#2563eb', letterSpacing: 0.3 },
  dayHeaders: {
    flexDirection: 'row',
    backgroundColor: '#fff',
    paddingHorizontal: 4,
  ...
  formTitle: { fontSize: 15, fontWeight: '700', color: '#374151', marginBottom: 10 },
