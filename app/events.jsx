...
import EventDetailItem, { eventTypeLabel, EVENT_TYPES } from '../components/EventDetailItem';
...
export default function EventsScreen() {
  ...
  useEffect(() => {
    navigation.setOptions({
      headerRight: () => (
        <TouchableOpacity style={styles.headerFilter} onPress={() => setCompanyPickerVisible(true)}>
  ...
  const [formCompanyPickerVisible, setFormCompanyPickerVisible] = useState(false);
  ...
  const futureEvents = events
    ...
    .filter((e) => !selectedCompany || e.company?.id === parseInt(selectedCompany))
    .sort((a, b) => parseEventTime(a.start_time) - parseEventTime(b.start_time));
      ...
      {(() => {
        ...
        return (
          <ScrollView style={styles.list} stickyHeaderIndices={stickyIndices}>
            {futureEvents.length === 0 && <Text style={styles.empty}>No upcoming events</Text>}
            {children}
          </ScrollView>
        );
      })()}
              ...
              >
                <View
                  style={[
                    ...
                    {
                      backgroundColor:
                        EVENT_TYPES.find((t) => t.value === form.event_type)?.color || '#6b7280',
                    },
                  ]}
                />
              ...
              >
                <Text style={styles.formDropdownText}>
                  {showNewCompany
                    ? '+ New Company'
                    : form.company_id
                      ? companies.find((c) => c.id.toString() === form.company_id)?.name || 'None'
                      : 'None'}
                </Text>
                <Text style={styles.filterDropdownChevron}>▾</Text>
              </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  dateSeparatorText: { fontSize: 12, fontWeight: '700', color: '#1e40af', letterSpacing: 0.3 },
  ...
  pickerDot: { width: 10, height: 10, borderRadius: 5, marginRight: 10, flexShrink: 0 },
  primaryBtn: {
    flex: 1,
