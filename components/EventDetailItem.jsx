...
export const eventTypeColor = (type, isArchived = false) => {
  const baseColor = EVENT_TYPES.find((t) => t.value === type)?.color || '#6b7280';
  return isArchived ? `${baseColor}80` : baseColor;
};
export const eventTypeLabel = (type) => EVENT_TYPES.find((t) => t.value === type)?.label || type;
...
export default function EventDetailItem({ event, onEdit, onDelete, onView }) {
  ...
  return (
    <View style={[styles.eventItem, isArchived && styles.archivedItem]}>
      <View
        style={[styles.eventDot, { backgroundColor: eventTypeColor(event.event_type, isArchived) }]}
      />
      <View style={styles.eventDetails}>
        <View style={styles.eventTitleRow}>
          <Text style={[styles.eventTitle, isArchived && styles.archivedText]}>{event.title}</Text>
          {!!event.selected_emoji && (
            <Text style={styles.eventEmoji}>
              {EMOJI_OPTIONS.find((e) => e.label === event.selected_emoji)?.emoji}
            </Text>
          )}
        </View>
        <Text style={styles.eventMeta}>
          {format(d, 'h:mm a')}
          {event.end_time ? ` – ${format(parseEventTime(event.end_time), 'h:mm a')}` : ''}
          {' · '}
      ...
      <View style={styles.eventActions}>
        {onView && (
          <TouchableOpacity onPress={() => onView(event)} style={styles.iconBtn}>
            <Eye size={15} color="#6b7280" />
          </TouchableOpacity>
        )}
        {onEdit && (
          <TouchableOpacity onPress={() => onEdit(event)} style={styles.iconBtn}>
            <Pencil size={15} color="#6b7280" />
          </TouchableOpacity>
        )}
        {onDelete && (
          <TouchableOpacity onPress={() => onDelete(event.id)} style={styles.iconBtn}>
